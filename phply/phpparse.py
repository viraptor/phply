...
def p_statement_foreach(p):
    ...
    if p[6] is None:
        ...
        p[0] = ast.Foreach(p[3], p[5], p[6], p[8], lineno=p.lineno(1), linespan=p.linespan(1))
...
def p_statement_try(p):
    ...
    p[0] = ast.Try(p[3], [ast.Catch(p[7], ast.Variable(p[8], lineno=p.lineno(8), linespan=p.linespan(8)),
                                    p[11], lineno=p.lineno(5), linespan=p.linespan(5))] + p[13],
...
def p_class_declaration_statement(p):
    ...
    if len(p) == 8:
        p[0] = ast.Class(p[2], p[1], p[3], p[4], p[6], lineno=p.lineno(2), linespan=p.linespan(2))
    else:
...
def p_expr_array(p):
    ...
    p[0] = ast.Array(p[3], lineno=p.lineno(1), linespan=p.linespan(1))
