...
class PHPCommon(object):
    ...
    def process_php_string_escapes(self, s):
    ...
    def p_top_statement_namespace(self, p):
    ...
    def p_constant_declarations(self, p):
    ...
    def p_statement_foreach(self, p):
    ...
    def p_statement_inline_html(self, p):
    ...
    def p_additional_catches(self, p):
    ...
    def p_declare_statement(self, p):
    ...
    def p_else_single(self, p):
        '''else_single : empty
                       | ELSE statement'''
        if len(p) == 3:
            p[0] = ast.Else(p[2], lineno=p.lineno(1))

    def p_new_elseif_list(self, p):
    ...
    def p_non_empty_for_expr(self, p):
    ...
    def p_foreach_optional_arg(self, p):
    ...
    def p_switch_case_list_colon(self, p):
    ...
    def p_case_separator(self, p):
        '''case_separator : COLON
                          | SEMI'''
        pass

    def p_global_var_list(self, p):
    ...
    def p_extends_from(self, p):
        '''extends_from : empty
                        | EXTENDS fully_qualified_class_name'''
        if len(p) == 3:
            p[0] = p[2]

    def p_fully_qualified_class_name(self, p):
    ...
    def p_trait_modifiers_list(self, p):
    ...
    def p_trait_statement_list(self, p):
    ...
    def p_class_statement_list(self, p):
    ...
    def p_class_variable_declaration_initial(self, p):
    ...
    def p_class_variable_declaration_no_initial(self, p):
    ...
    def p_class_constant_declaration(self, p):
    ...
    def p_variable_modifiers_non_empty(self, p):
        'variable_modifiers : non_empty_member_modifiers'
        p[0] = p[1]

    def p_variable_modifiers_var(self, p):
        'variable_modifiers : VAR'
        p[0] = []

    def p_method_modifiers_non_empty(self, p):
        'method_modifiers : non_empty_member_modifiers'
        p[0] = p[1]

    def p_method_modifiers_empty(self, p):
    ...
    def p_non_empty_member_modifiers(self, p):
    ...
    def p_expr_yield(self, p):
        ...
        if len(p) == 2:
            ...
            p[0] = ast.Yield(p[3], lineno=p.lineno(1))

    def p_class_name_reference(self, p):
    ...
    def p_class_name_static(self, p):
        'class_name : STATIC'
        p[0] = p[1].lower()

    def p_dynamic_class_name_reference(self, p):
    ...
    def p_dynamic_class_name_variable_properties(self, p):
    ...
    def p_dynamic_class_name_variable_property(self, p):
        'dynamic_class_name_variable_property : OBJECT_OPERATOR object_property'
        p[0] = p[2]

    def p_ctor_arguments(self, p):
    ...
    def p_expr_clone(self, p):
        'expr : CLONE expr'
        p[0] = ast.Clone(p[2], lineno=p.lineno(1))

    def p_expr_list_assign(self, p):
    ...
    def p_base_variable_with_function_calls(self, p):
    ...
    def p_function_call_static(self, p):
    ...
    def p_function_call_static_dynamic_name(self, p):
    ...
    def p_variable_without_objects(self, p):
        'variable_without_objects : simple_indirect_reference'
        p[0] = p[1]

    def p_expr_scalar(self, p):
        'expr : scalar'
        p[0] = p[1]
    ...
    def p_non_empty_array_pair_list_item(self, p):
    ...
    def p_non_empty_array_pair_list_pair(self, p):
    ...
    def p_possible_comma(self, p):
        '''possible_comma : empty
                          | COMMA'''
        pass

    def p_function_call_parameter_list(self, p):
    ...
    def p_function_call_parameter_list_empty(self, p):
        'function_call_parameter_list : empty'
        p[0] = []

    def p_function_call_parameter(self, p):
    ...
    def p_expr_print(self, p):
        'expr : PRINT expr'
        p[0] = ast.Print(p[2], lineno=p.lineno(1))

    def p_expr_silence(self, p):
        'expr : AT expr'
    ...
    def p_nowdoc_text_content(self, p):
    ...
    def p_scalar_string_varname(self, p):
        'scalar : STRING_VARNAME'
        p[0] = ast.Variable('$' + p[1], lineno=p.lineno(1))

    def p_scalar_namespace_name(self, p):
    ...
    def p_common_scalar_dnumber(self, p):
        'common_scalar : DNUMBER'
        p[0] = float(p[1])

    def p_common_scalar_string(self, p):
    ...
    def p_common_scalar_magic_class(self, p):
        'common_scalar : CLASS_C'
        p[0] = ast.MagicConstant(p[1].upper(), None, lineno=p.lineno(1))

    def p_common_scalar_magic_method(self, p):
        'common_scalar : METHOD_C'
        p[0] = ast.MagicConstant(p[1].upper(), None, lineno=p.lineno(1))

    def p_common_scalar_magic_func(self, p):
        'common_scalar : FUNC_C'
        p[0] = ast.MagicConstant(p[1].upper(), None, lineno=p.lineno(1))

    def p_common_scalar_magic_ns(self, p):
        'common_scalar : NS_C'
        p[0] = ast.MagicConstant(p[1].upper(), None, lineno=p.lineno(1))

    def p_static_scalar(self, p):
    ...
    def p_multiple_encapsed(self, p):
    ...
    def p_static_scalar_namespace_name(self, p):
    ...
    def p_static_scalar_unary_op(self, p):
        '''static_scalar : PLUS static_scalar
                         | MINUS static_scalar'''
        p[0] = ast.UnaryOp(p[1], p[2], lineno=p.lineno(1))

    def p_static_scalar_array(self, p):
    ...
    def p_static_array_pair_list(self, p):
    ...
    def p_static_non_empty_array_pair_list_item(self, p):
    ...
    def p_static_non_empty_array_pair_list_pair(self, p):
    ...
    def p_static_expr_group(self, p):
    ...
    def p_encaps_list_string(self, p):
        ...
        if p[1] == '':
            ...
            if isinstance(p[1], string_type):
                ...
                p[0] = ast.BinaryOp('.', p[1].left, p[1].right + self.process_php_string_escapes(p[2]),
                                    lineno=p[1].lineno)
            else:
                # worst case - insert a binaryop
                p[0] = ast.BinaryOp('.', p[1], self.process_php_string_escapes(p[2]),
                                    lineno=p.lineno(2))

    def p_encaps_var(self, p):
        'encaps_var : VARIABLE'
        p[0] = ast.Variable(p[1], lineno=p.lineno(1))

    def p_encaps_var_array_offset(self, p):
        'encaps_var : VARIABLE LBRACKET encaps_var_offset RBRACKET'
        p[0] = ast.ArrayOffset(ast.Variable(p[1], lineno=p.lineno(1)), p[3],
                               lineno=p.lineno(2))

    def p_encaps_var_object_property(self, p):
        'encaps_var : VARIABLE OBJECT_OPERATOR STRING'
        p[0] = ast.ObjectProperty(ast.Variable(p[1], lineno=p.lineno(1)), p[3],
                                  lineno=p.lineno(2))

    def p_encaps_var_dollar_curly_expr(self, p):
        'encaps_var : DOLLAR_OPEN_CURLY_BRACES expr RBRACE'
        p[0] = p[2]

    def p_encaps_var_dollar_curly_array_offset(self, p):
    ...
    def p_encaps_var_curly_variable(self, p):
        'encaps_var : CURLY_OPEN variable RBRACE'
        p[0] = p[2]

    def p_encaps_var_offset_string(self, p):
        'encaps_var_offset : STRING'
        p[0] = p[1]

    def p_encaps_var_offset_num_string(self, p):
        'encaps_var_offset : NUM_STRING'
        p[0] = int(p[1])

    def p_encaps_var_offset_variable(self, p):
...
class PHP5Parser(PHPCommon):
    precedence = PHPCommon.precedence

class PHP7Parser(PHPCommon):
    precedence = PHPCommon.precedence[:6] + [
        ('right', 'YIELD'),
        ('right', 'YIELD_FROM'),
    ] + PHPCommon.precedence[6:]
...
# Build the grammar
def make_parser(debug=False, version=5):
    mod = version_map[version]()
    return yacc.yacc(debug=debug, module=mod)
